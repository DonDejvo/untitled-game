...
public class PlayerController extends EntityController {
    ...
    public PlayerController(Level level, Camera camera, BoxCollider3D collider, Inventory inventory, PathFinder pathFinder) {
        super(level, collider, new String[]{ "fixed", "enemy" }, pathFinder, 100);
        this.cameraOffset = new Vector3d(0, 1.5, 0);
        this.camera = camera;
    ...
    */
    private void updatePlaying(double dt) {
        ...
        if(focusedObject != null) {
            Canvas canvas = gameObject.getGame().getCanvas();
            canvas.setColor(Color.WHITE);
            canvas.setFontSize(canvas.getHeight() / 26f);
            TextAlign align = canvas.getTextAlign();
            canvas.setTextAlign(TextAlign.CENTER);
            canvas.text(focusedObject.getComponent(Controller.class, "Controller").getName(), canvas.getWidth() / 2f, canvas.getHeight() / 24f);
    ...
    */
    private void updateGun() {
        ...
        if(gun != null) {
            ...
            gun.transform.position.add(new Vector3d(0, 1, 1).rotateY(yaw));

            Vector3d rayHitPos = raycast(camera.getGameObject().transform.position, direction, 100, 1);
            ...
            if(rayHitPos != null) {
                ...
                DebugDraw.get().addCross(rayHitPos, 1, Color.YELLOW);
            } else {
